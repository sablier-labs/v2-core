...
abstract contract DeploymentLogger is BaseScript {
    ...
    function appendToFileDeployedAddresses(address lockup, address nftDescriptor, address batchLockup) internal {
        ...
        string memory batchLockupLine =
            _getContractLine({ contractName: "SablierBatchLockup", contractAddress: batchLockup.toHexString() });
        _appendToFile(batchLockupLine);

        string memory lockupLine =
            _getContractLine({ contractName: "SablierLockup", contractAddress: lockup.toHexString() });
        _appendToFile(lockupLine);

        string memory nftDescriptorLine =
            _getContractLine({ contractName: "SablierNFTDescriptor", contractAddress: nftDescriptor.toHexString() });
