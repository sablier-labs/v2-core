...
contract GetTranches_Integration_Concrete_Test is Lockup_Tranched_Integration_Concrete_Test {
    function test_RevertGiven_Null() external {
        expectRevert_Null({ callData: abi.encodeCall(lockup.getTranches, streamIds.nullStream) });
    }
    ...
    function test_GivenTranchedModel() external givenNotNull {
        LockupTranched.Tranche[] memory actualTranches = lockup.getTranches(streamIds.defaultStream);
