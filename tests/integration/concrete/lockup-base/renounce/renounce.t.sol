...
abstract contract Renounce_Integration_Concrete_Test is Integration_Test {
    ...
    function test_RevertWhen_DelegateCall() external {
        expectRevert_DelegateCall({ callData: abi.encodeCall(lockup.renounce, streamIds.defaultStream) });
    }

    function test_RevertGiven_Null() external whenNoDelegateCall {
        expectRevert_Null({ callData: abi.encodeCall(lockup.renounce, streamIds.nullStream) });
    }

    function test_RevertGiven_DEPLETEDStatus() external whenNoDelegateCall givenNotNull givenColdStream {
        expectRevert_DEPLETEDStatus({ callData: abi.encodeCall(lockup.renounce, streamIds.defaultStream) });
    }

    function test_RevertGiven_CANCELEDStatus() external whenNoDelegateCall givenNotNull givenColdStream {
        expectRevert_CANCELEDStatus({ callData: abi.encodeCall(lockup.renounce, streamIds.defaultStream) });
    }

    function test_RevertGiven_SETTLEDStatus() external whenNoDelegateCall givenNotNull givenColdStream {
        expectRevert_SETTLEDStatus({ callData: abi.encodeCall(lockup.renounce, streamIds.defaultStream) });
    }
    ...
    function test_RevertWhen_CallerNotSender() external whenNoDelegateCall givenNotNull givenWarmStreamRenounce {
        expectRevert_CallerMaliciousThirdParty({ callData: abi.encodeCall(lockup.renounce, streamIds.defaultStream) });
    }
        ...
        whenCallerSender
    {
        ...
        lockup.renounce(streamIds.notCancelableStream);
    }
