...
abstract contract Withdraw_Integration_Concrete_Test is Integration_Test {
    ...
    function test_RevertWhen_DelegateCall() external {
        ...
        expectRevert_DelegateCall({
            callData: abi.encodeCall(lockup.withdraw, (streamIds.defaultStream, users.recipient, withdrawAmount))
        });
    }
    ...
    function test_RevertGiven_Null() external whenNoDelegateCall {
        uint128 withdrawAmount = defaults.WITHDRAW_AMOUNT();
        expectRevert_Null({
            callData: abi.encodeCall(lockup.withdraw, (streamIds.nullStream, users.recipient, withdrawAmount))
        });
    }
    ...
    function test_RevertGiven_DEPLETEDStatus() external whenNoDelegateCall givenNotNull {
        expectRevert_DEPLETEDStatus({
            callData: abi.encodeCall(
                lockup.withdraw, (streamIds.defaultStream, users.recipient, defaults.WITHDRAW_AMOUNT())
            )
        });
    }
    ...
    function test_RevertWhen_WithdrawalAddressZero() external whenNoDelegateCall givenNotNull givenNotDEPLETEDStatus {
        uint128 withdrawAmount = defaults.WITHDRAW_AMOUNT();
        vm.expectRevert(
            abi.encodeWithSelector(Errors.SablierLockupBase_WithdrawToZeroAddress.selector, streamIds.defaultStream)
        );
        lockup.withdraw({ streamId: streamIds.defaultStream, to: address(0), amount: withdrawAmount });
    }
    ...
    {
        vm.expectRevert(
            abi.encodeWithSelector(Errors.SablierLockupBase_WithdrawAmountZero.selector, streamIds.defaultStream)
        );
        lockup.withdraw({ streamId: streamIds.defaultStream, to: users.recipient, amount: 0 });
    }
        ...
        whenNonZeroWithdrawAmount
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.SablierLockupBase_Overdraw.selector, streamIds.defaultStream, MAX_UINT128, withdrawableAmount
            )
        );
        lockup.withdraw({ streamId: streamIds.defaultStream, to: users.recipient, amount: MAX_UINT128 });
    }
    ...
    modifier whenWithdrawalAddressNotRecipient(bool isCallerRecipient) {
        if (!isCallerRecipient) {
            ...
            resetPrank({ msgSender: caller });
            lockup.transferFrom(caller, users.eve, streamIds.defaultStream);
            _;
        } else {
        ...
        whenWithdrawalAddressNotRecipient(false)
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.SablierLockupBase_WithdrawalAddressNotRecipient.selector,
                streamIds.defaultStream,
                caller,
                users.alice
            )
        );
        lockup.withdraw({ streamId: streamIds.defaultStream, to: users.alice, amount: withdrawAmount });
    }
        ...
        whenWithdrawalAddressNotRecipient(true)
    {
        ...
        uint128 previousWithdrawnAmount = lockup.getWithdrawnAmount(streamIds.defaultStream);
        ...
        vm.expectEmit({ emitter: address(lockup) });
        emit IERC4906.MetadataUpdate({ _tokenId: streamIds.defaultStream });
        ...
        whenWithdrawalAddressRecipient
    {
        ...
        // Make the withdrawal.
        lockup.withdraw({ streamId: streamIds.defaultStream, to: users.recipient, amount: defaults.WITHDRAW_AMOUNT() });
        ...
        whenCallerSender
    {
        ...
        // Make the withdrawal.
        lockup.withdraw({ streamId: streamIds.defaultStream, to: users.recipient, amount: defaults.DEPOSIT_AMOUNT() });
        ...
        address actualNFTowner = lockup.ownerOf({ tokenId: streamIds.defaultStream });
        ...
        givenEndTimeInFuture
    {
        ...
        uint128 withdrawAmount = lockup.withdrawableAmountOf(streamIds.defaultStream);
        ...
        givenNotCanceledStream
    {
        ...
        uint128 withdrawAmount = lockup.withdrawableAmountOf(streamIds.notAllowedtoHookStream);
        vm.expectCall({
            callee: address(recipientGood),
            data: abi.encodeCall(
                ISablierLockupRecipient.onSablierLockupWithdraw,
                (streamIds.notAllowedtoHookStream, users.sender, address(recipientInterfaceIDIncorrect), withdrawAmount)
            ),
            count: 0
        });
        ...
        // Make the withdrawal.
        lockup.withdraw({
            streamId: streamIds.notAllowedtoHookStream,
            to: address(recipientInterfaceIDIncorrect),
            amount: withdrawAmount
        });

        // It should update the withdrawn amount.
        uint128 actualWithdrawnAmount = lockup.getWithdrawnAmount(streamIds.notAllowedtoHookStream);
        uint128 expectedWithdrawnAmount = withdrawAmount;
        assertEq(actualWithdrawnAmount, expectedWithdrawnAmount, "withdrawnAmount");
    }
        ...
        givenRecipientAllowedToHook
    {
        ...
        // Make the withdrawal.
        lockup.withdraw({
            streamId: streamIds.recipientRevertStream,
        ...
        whenNonRevertingRecipient
    {
        ...
        // Cancel the stream.
        lockup.withdraw({
            streamId: streamIds.recipientInvalidSelectorStream,
        ...
        whenHookReturnsValidSelector
    {
        ...
        vm.expectCall(
            ...
            abi.encodeCall(
                ISablierLockupRecipient.onSablierLockupWithdraw,
                (streamIds.recipientReentrantStream, users.sender, address(recipientReentrant), withdrawAmount)
            )
        );

        // It should make multiple withdrawals.
        lockup.withdraw({
            streamId: streamIds.recipientReentrantStream,
        ...
        whenHookReturnsValidSelector
    {
        ...
        vm.expectCall(
            ...
            abi.encodeCall(
                ISablierLockupRecipient.onSablierLockupWithdraw,
                (streamIds.recipientGoodStream, users.sender, address(recipientGood), withdrawAmount)
            )
        );
        ...
        // Make the withdrawal.
        lockup.withdraw({ streamId: streamIds.recipientGoodStream, to: address(recipientGood), amount: withdrawAmount });
        ...
        // It should update the withdrawn amount.
        uint128 actualWithdrawnAmount = lockup.getWithdrawnAmount(streamIds.recipientGoodStream);
        uint128 expectedWithdrawnAmount = withdrawAmount;
        assertEq(actualWithdrawnAmount, expectedWithdrawnAmount, "withdrawnAmount");
    }
}
