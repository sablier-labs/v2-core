...
contract Burn_Integration_Concrete_Test is Integration_Test {
    function test_RevertWhen_DelegateCall() external {
        expectRevert_DelegateCall({ callData: abi.encodeCall(lockup.burn, streamIds.defaultStream) });
    }

    function test_RevertGiven_Null() external whenNoDelegateCall {
        expectRevert_Null({ callData: abi.encodeCall(lockup.burn, streamIds.nullStream) });
    }

    function test_RevertGiven_PENDINGStatus() external whenNoDelegateCall givenNotNull givenNotDepletedStream {
        vm.warp({ newTimestamp: getBlockTimestamp() - 1 seconds });
        vm.expectRevert(
            abi.encodeWithSelector(Errors.SablierLockupBase_StreamNotDepleted.selector, streamIds.defaultStream)
        );
    ...
    function test_RevertWhen_CallerMaliciousThirdParty()
        ...
        whenNoDelegateCall
        givenNotNull
        givenDepletedStream(lockup, streamIds.defaultStream)
        whenCallerNotRecipient
    {
        expectRevert_CallerMaliciousThirdParty({ callData: abi.encodeCall(lockup.burn, streamIds.defaultStream) });
    }
        ...
        whenCallerNotRecipient
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.SablierLockupBase_Unauthorized.selector, streamIds.defaultStream, users.sender
            )
        );
        lockup.burn(streamIds.defaultStream);
    }
        ...
        whenCallerNotRecipient
    {
        ...
        _test_Burn(streamIds.defaultStream);
    }
    ...
    function test_GivenNonTransferableNFT()
        ...
        whenNoDelegateCall
        givenNotNull
        givenDepletedStream(lockup, streamIds.notTransferableStream)
        whenCallerRecipient
        givenNFTExists
    {
        _test_Burn(streamIds.notTransferableStream);
    }
