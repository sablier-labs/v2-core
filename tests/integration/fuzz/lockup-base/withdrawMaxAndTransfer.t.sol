...
contract WithdrawMaxAndTransfer_Integration_Fuzz_Test is Integration_Test {
        ...
        givenTransferableStream
    {
        ...
        // Expect the relevant event to be emitted.
        vm.expectEmit({ emitter: address(lockup) });
        emit IERC721.Transfer({ from: users.recipient, to: newRecipient, tokenId: streamIds.defaultStream });

        // Make the max withdrawal and transfer the NFT.
        lockup.withdrawMaxAndTransfer({ streamId: streamIds.defaultStream, newRecipient: newRecipient });
