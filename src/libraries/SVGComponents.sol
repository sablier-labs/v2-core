...
library SVGComponents {
    ...
    {
        BoxVars memory vars;
        vars.hasProgress = boxType == BoxType.PROGRESS;

        vars.circle = vars.hasProgress
            ? string.concat(
                '<g> <circle cx="45" cy="45" r="22" fill="none" stroke="',
                COLOR_BACKGROUND,
                '" stroke-width="10" /> <circle transform="rotate(-95)" transform-origin="45 45" cx="45" cy="45" r="22" fill="none" stroke="',
                colorAccent,
        ...
        vars.left = 20 + (vars.hasProgress ? 70 : 0);
        vars.width = 20 * 2 + (vars.hasProgress ? 70 + 20 : 0) + getTextLength(text);

        vars.element = string.concat(
            '<g id="',
            boxTypeToString(boxType),
            '"> <rect height="90" width="',
            vars.width.toString(),
            '" rx="15" ry="15" fill="white" fill-opacity="0.03" stroke="white" stroke-opacity="0.1" stroke-width="4" />',
            vars.circle,
            '<text x="',
            vars.left.toString(),
            '" y="55" fill="white" font-size="28px" lengthAdjust="spacing" font-family="\'Trebuchet MS\', \'Courier New\', \'Arial\', \'monospace\'">',
            text,
            "</text></g>"
        );

        return (vars.element, vars.width);
    }

    function boxTypeToString(BoxType boxType) internal pure returns (string memory boxTypeString) {
        if (boxType == BoxType.PROGRESS) {
            return "progress";
        } else if (boxType == BoxType.DURATION) {
            return "duration";
        } else if (boxType == BoxType.CURVE) {
            boxTypeString = "curve";
        }
    }

    function getTextLength(string memory text) internal pure returns (uint256 length) {
        bytes memory textBytes = bytes(text);

        for (uint256 i = 0; i < textBytes.length;) {
            if (
                textBytes[i] == "." || textBytes[i] == "," || textBytes[i] == "1" || textBytes[i] == "i"
                    || textBytes[i] == "I"
            ) {
                length += 6;
            } else {
    ...
    function words(string memory offset, string memory text) internal pure returns (string memory) {
        return string.concat(
            ...
            offset,
            '" href="#boxOutline" fill="white" fill-opacity="0.8" font-size="28px" lengthAdjust="spacing" font-family="\'Trebuchet MS\', \'Courier New\', \'Arial\', \'monospace\'">',
