...
library SVGComponents {
    ...
    string internal constant HOURGLASS_FRAME =
        '<g fill="none" stroke="url(#HourglassStroke)" stroke-linecap="round" stroke-miterlimit="10" stroke-width="4"><line x1="184.584" y1="206.823" x2="184.585" y2="537.579"/><line x1="218.181" y1="218.118" x2="218.181" y2="562.537"/><line x1="481.818" y1="218.142" x2="481.819" y2="562.428"/><line x1="515.415" y1="207.352" x2="515.416" y2="537.579"/><g><path d="m565.641,107.28c0,9.537-5.56,18.629-15.676,26.973h-.023c-9.204,7.596-22.194,14.562-38.197,20.592-39.504,14.936-97.325,24.355-161.733,24.355-90.48,0-167.948-18.582-199.953-44.948h-.023c-10.115-8.344-15.676-17.437-15.676-26.973,0-39.735,96.554-71.921,215.652-71.921s215.629,32.185,215.629,71.921Z"/><path d="m134.36,161.203c0,39.735,96.554,71.921,215.652,71.921s215.629-32.186,215.629-71.921"/><line x1="134.36" y1="161.203" x2="134.36" y2="107.28"/><line x1="565.64" y1="161.203" x2="565.64" y2="107.28"/></g><g><path d="m184.582,492.656c-31.354,12.485-50.223,28.58-50.223,46.142,0,9.536,5.564,18.627,15.677,26.969h.022c8.503,7.005,20.213,13.463,34.524,19.159,9.999,3.991,21.269,7.609,33.597,10.788,36.45,9.407,82.181,15.002,131.835,15.002s95.363-5.595,131.807-15.002c10.847-2.79,20.867-5.926,29.924-9.349,1.244-.467,2.473-.942,3.673-1.424,14.326-5.696,26.035-12.161,34.524-19.173h.022c10.114-8.342,15.677-17.433,15.677-26.969,0-17.562-18.869-33.665-50.223-46.15"/><path d="m134.36,592.72c0,39.735,96.554,71.921,215.652,71.921s215.629-32.186,215.629-71.921"/><line x1="134.36" y1="592.72" x2="134.36" y2="538.797"/><line x1="565.64" y1="592.72" x2="565.64" y2="538.797"/></g><polyline points="218.185 481.901 218.231 481.854 350.015 350.026 481.822 218.152"/><polyline points="481.822 481.901 481.798 481.877 481.775 481.854 350.015 350.026 218.185 218.129"/><path d="m184.58,537.58c0,5.45,4.27,10.65,12.03,15.42h.02c5.51,3.39,12.79,6.55,21.55,9.42,30.21,9.9,78.02,16.28,131.83,16.28,49.41,0,93.76-5.38,124.06-13.92,2.7-.76,5.29-1.54,7.75-2.35,8.77-2.87,16.05-6.04,21.56-9.43h0c7.76-4.77,12.04-9.97,12.04-15.42"/></g>';

    string internal constant HOURGLASS_LOWER_BULB =
        '<path d="m481.46,481.54v81.01c-2.35.77-4.82,1.51-7.39,2.23-30.3,8.54-74.65,13.92-124.06,13.92-53.6,0-101.24-6.33-131.47-16.16v-81l46.3-46.31h170.33l46.29,46.31Z" fill="url(#SandRight)"/><path d="m435.17,435.23c0,1.17-.46,2.32-1.33,3.44-7.11,9.08-41.93,15.98-83.81,15.98s-76.7-6.9-83.82-15.98c-.87-1.12-1.33-2.27-1.33-3.44v-.04l8.34-8.35.01-.01c13.72-6.51,42.95-11.02,76.8-11.02s62.97,4.49,76.72,11l8.42,8.42Z" fill="url(#SandLeft)"/>';

    string internal constant HOURGLASS_UPPER_BULB =
        '<polygon points="350 350.026 415.03 284.978 285 284.978 350 350.026" fill="url(#SandRight)"/><path d="m416.341,281.975c0,.914-.354,1.809-1.035,2.68-5.542,7.076-32.661,12.45-65.28,12.45-32.624,0-59.738-5.374-65.28-12.45-.681-.872-1.035-1.767-1.035-2.68,0-.914.354-1.808,1.035-2.676,5.542-7.076,32.656-12.45,65.28-12.45,32.619,0,59.738,5.374,65.28,12.45.681.867,1.035,1.762,1.035,2.676Z" fill="url(#SandLeft)"/>';
    ...
    string internal constant SIGN_GTE = "&gt;=";
    ...
    {
        string memory label = getCardLabel(cardType);
        uint256 labelWidth = getWidth({ text: label, isSmall: true });
        uint256 valueWidth = getWidth({ text: value, isSmall: false });
        width = Math.max(labelWidth, valueWidth) + 40;

        // Only the Progress card has a progress circle.
        ...
        card_ = string.concat(
            ...
            cardType == CardType.PROGRESS ? progressCircle(accentColor, progress) : "",
    ...
    function getCardLabel(CardType cardType) internal pure returns (string memory label) {
        if (cardType == CardType.PROGRESS) {
            label = "Progress";
    ...
    function getWidth(string memory text, bool isSmall) internal pure returns (uint256 width) {
        uint256 length = bytes(text).length;
        for (uint256 i = 0; i < length;) {
            unchecked {
                width = width + (isSmall ? 15 : 16); // 15px or 16px per character
                i += 1;
            }
        }
    }
    ...
    function gradients(string memory accentColor) internal pure returns (string memory) {
        ...
        string memory sandLeft = string.concat(
            '<linearGradient id="SandLeft" x1="0%" y1="0%">',
            '<stop offset="0%" stop-color="',
            accentColor,
            '"/>',
            '<stop offset="100%" stop-color="',
        ...
        string memory sandRight = string.concat(
            '<linearGradient id="SandRight" x1="100%" y1="100%">',
        ...
        return string.concat(radialGlow, hourglassStroke, sandLeft, sandRight);
    }

    function hourglass(bool isDepleted) internal pure returns (string memory) {
        // The upper bulb is either empty or filled depending on whether the stream is depleted.
        return string.concat(
            '<g id="Hourglass">',
            HOURGLASS_BACKGROUND_CIRCLE,
            HOURGLASS_FILL,
            HOURGLASS_FRAME,
            isDepleted ? "" : HOURGLASS_UPPER_BULB,
            HOURGLASS_LOWER_BULB,
            "</g>"
        );
    }

    function progressCircle(string memory accentColor, uint256 progress) internal pure returns (string memory) {
        uint256 labelWidth = getWidth({ text: "Progress", isSmall: true });
        string memory offset = (labelWidth + 40).toString();
        return string.concat(
            '<g fill="none">',
            '<circle cx="',
            offset,
            ...
            BACKGROUND_COLOR,
            '" stroke-width="10"/>',
            '<circle transform="rotate(-95)" transform-origin="',
            offset,
            ' 50" cx="',
            offset,
            '" cy="50" r="22" stroke="',
            accentColor,
            '" stroke-dasharray="',
            ((progress * 138) / 100).toString(), // 138 is the circumference of the circle, 2 * π * 22
            ', 138" stroke-linecap="round" stroke-width="5"/>',
            "</g>"
        );
    }
}
