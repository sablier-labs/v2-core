...
/// @title SablierV2
/// @dev Abstract contract that implements the ISablierV2 interface and other common logic.
    ...
    ISablierV2 // three dependencies
{
    ...
    modifier isActiveStream(uint256 streamId) {
        if (getStatus(streamId) != Status.ACTIVE) {
            revert Errors.SablierV2_StreamNotActive(streamId);
        }
        _;
    }
    ...
    function cancelMultiple(uint256[] calldata streamIds) external override {
        ...
        for (uint256 i = 0; i < count; ) {
            ...
            // Effects and Interactions: cancel the stream.
            // We cancel this stream only if the `streamId` points to a stream that is active and cancelable.
    ...
    function claimProtocolRevenues(IERC20 token) external override onlyAdmin {
        ...
        if (protocolRevenues == 0) {
            revert Errors.SablierV2_NoProtocolRevenues(token);
        }
    ...
    function withdrawMultiple(uint256[] calldata streamIds, address to, uint128[] calldata amounts) external override {
        ...
        for (uint256 i = 0; i < streamIdsCount; ) {
            ...
            // If the `streamId` does not point to an active stream, we simply skip it.
            if (getStatus(streamId) == Status.ACTIVE) {
