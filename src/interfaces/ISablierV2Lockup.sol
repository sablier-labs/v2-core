    ...
    IERC721Metadata // two dependencies
{
    ...
    /// @notice Queries the amount deposited in the stream, denoted in units of the asset's decimals.
    /// @dev Reverts if `streamId` points to a null stream.
    /// @param streamId The id of the stream to make the query for.
    ...
    /// @notice Queries the amount withdrawn from the stream, denoted in units of the asset's decimals.
    /// @dev Reverts if `streamId` points to a null stream.
    /// @param streamId The id of the stream to make the query for.
    ...
    /// @notice Calculates the amount that the sender would be paid if the stream were to be canceled, denoted
    ...
    /// - `streamId` must point to a stream that is depleted.
    /// - The NFT must exist.
    ...
    /// Requirements:
    /// - The call must not be a delegate call.
    /// - Each value in `streamIds` must point to a stream that is active or cancelable.
    ...
    /// Requirements:
    /// - The call must not be a delegate call.
    /// - `streamId` must point to a stream that is either active or canceled.
    ...
    /// - Each value in `streamId` must point to a stream that is either active or canceled.
