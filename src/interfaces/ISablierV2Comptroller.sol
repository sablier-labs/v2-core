...
interface ISablierV2Comptroller is IAdminable {
    /// @notice Queries the protocol fee charged on all Sablier V2 streams created with the provided asset, as an
    ...
    /// @notice Sets a new protocol fee that will be charged on all streams created with the provided asset.
    ///
    /// @dev Emits a {SetProtocolFee} event.
    ///
    /// Notes:
    /// - The fee is not in units of the asset's decimals, instead it follows the UD60x18 number format. Refer to the
    ...
    ///
    /// @param asset The contract address of the ERC-20 asset to make the query for.
    /// @param newFee The new protocol fee to set, as an UD60x18 number where 100% = 1e18.
    function setProtocolFee(IERC20 asset, UD60x18 newFee) external;
}
