...
/// @notice Creates and manages Lockup streams with dynamic distribution functions.
interface ISablierV2LockupDynamic is ISablierV2Lockup {
    ...
    /// @param sender The address from which to distribute the assets, which will have the ability to cancel the stream.
    ...
    /// @notice Retrieves the segments the protocol uses to compose the dynamic distribution function.
    /// @dev Reverts if `streamId` references a null stream.
    /// @param streamId The stream ID for the query.
