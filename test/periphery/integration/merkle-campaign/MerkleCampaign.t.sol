...
abstract contract MerkleCampaign_Integration_Test is Periphery_Test {
    ...
    function claim() internal {
        merkleBase.claim{ value: SABLIER_FEE }({
    ...
    function computeMerkleInstantAddress() internal view returns (address) {
        return
            computeMerkleInstantAddress(users.campaignOwner, defaults.MERKLE_ROOT(), defaults.EXPIRATION(), SABLIER_FEE);
    }

    function computeMerkleInstantAddress(address campaignOwner) internal view returns (address) {
        return computeMerkleInstantAddress(campaignOwner, defaults.MERKLE_ROOT(), defaults.EXPIRATION(), SABLIER_FEE);
    }

    function computeMerkleInstantAddress(address campaignOwner, uint40 expiration) internal view returns (address) {
        return computeMerkleInstantAddress(campaignOwner, defaults.MERKLE_ROOT(), expiration, SABLIER_FEE);
    }
    ...
    function computeMerkleInstantAddress(address campaignOwner, bytes32 merkleRoot) internal view returns (address) {
        return computeMerkleInstantAddress(campaignOwner, merkleRoot, defaults.EXPIRATION(), SABLIER_FEE);
    }
    ...
    function computeMerkleLLAddress() internal view returns (address) {
        return computeMerkleLLAddress(users.campaignOwner, defaults.MERKLE_ROOT(), defaults.EXPIRATION(), SABLIER_FEE);
    }

    function computeMerkleLLAddress(address campaignOwner) internal view returns (address) {
        return computeMerkleLLAddress(campaignOwner, defaults.MERKLE_ROOT(), defaults.EXPIRATION(), SABLIER_FEE);
    }

    function computeMerkleLLAddress(address campaignOwner, uint40 expiration) internal view returns (address) {
        return computeMerkleLLAddress(campaignOwner, defaults.MERKLE_ROOT(), expiration, SABLIER_FEE);
    }
    ...
    function computeMerkleLLAddress(address campaignOwner, bytes32 merkleRoot) internal view returns (address) {
        return computeMerkleLLAddress(campaignOwner, merkleRoot, defaults.EXPIRATION(), SABLIER_FEE);
    }
    ...
    function computeMerkleLTAddress() internal view returns (address) {
        return computeMerkleLTAddress(users.campaignOwner, defaults.MERKLE_ROOT(), defaults.EXPIRATION(), SABLIER_FEE);
    }

    function computeMerkleLTAddress(address campaignOwner) internal view returns (address) {
        return computeMerkleLTAddress(campaignOwner, defaults.MERKLE_ROOT(), defaults.EXPIRATION(), SABLIER_FEE);
    }

    function computeMerkleLTAddress(address campaignOwner, uint40 expiration) internal view returns (address) {
        return computeMerkleLTAddress(campaignOwner, defaults.MERKLE_ROOT(), expiration, SABLIER_FEE);
    }
    ...
    function computeMerkleLTAddress(address campaignOwner, bytes32 merkleRoot) internal view returns (address) {
        return computeMerkleLTAddress(campaignOwner, merkleRoot, defaults.EXPIRATION(), SABLIER_FEE);
    }
