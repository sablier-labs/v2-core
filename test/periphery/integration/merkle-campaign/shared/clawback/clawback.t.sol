...
abstract contract Clawback_Integration_Test is MerkleCampaign_Integration_Shared_Test {
    ...
    function test_WhenFirstClaimNotMade() external whenCallerRecipient(users.admin) {
        test_Clawback(users.admin);
    }

    function test_GivenSevenDaysNotPassed() external whenCallerRecipient(users.admin) whenFirstClaimMade {
    ...
    function test_Clawback(address to) internal {
        ...
        emit ISablierMerkleBase.Clawback({ admin: users.admin, to: to, amount: clawbackAmount });
