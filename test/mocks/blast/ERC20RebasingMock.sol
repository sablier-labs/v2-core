...
contract ERC20RebasingMock is ERC20("ERC20Rebasing Mock", "REB-MOCK"), IERC20Rebasing {
    ...
    function getClaimableAmount(address account) public view override returns (uint256 amount) {
        require(getConfiguration(account) == YieldMode.CLAIMABLE, "ERC20Rebasing: not claimable");
    ...
    function claim(address recipient, uint256 amount) public override returns (uint256 claimed) {
        require(recipient != address(0), "ERC20Rebasing: zero address");
        require(getConfiguration(msg.sender) == YieldMode.CLAIMABLE, "ERC20Rebasing: not claimable");
        require(_claimable[msg.sender] >= amount, "ERC20Rebasing: insufficient");
