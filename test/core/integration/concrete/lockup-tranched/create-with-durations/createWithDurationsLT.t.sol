...
contract CreateWithDurationsLT_Integration_Concrete_Test is Lockup_Tranched_Integration_Shared_Test {
    ...
    function test_RevertWhen_DelegateCall() external {
        bytes memory callData = abi.encodeCall(
            ISablierLockup.createWithDurationsLT, (defaults.createWithDurations(), defaults.tranchesWithDurations())
        );
    ...
    function test_RevertWhen_TrancheCountExceedsMaxValue() external whenNoDelegateCall {
        ...
        vm.expectRevert(abi.encodeWithSelector(Errors.SablierLockup_TrancheCountTooHigh.selector, 25_000));
        createDefaultStreamWithDurationsLT(tranches);
    }
