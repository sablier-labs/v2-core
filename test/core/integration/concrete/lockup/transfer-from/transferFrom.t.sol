...
abstract contract TransferFrom_Integration_Concrete_Test is Integration_Test, Lockup_Integration_Shared_Test {
    ...
    function test_RevertGiven_StreamNotTransferable() external {
        ...
        lockup.transferFrom({ from: users.recipient1, to: users.alice, tokenId: notTransferableStreamId });
    }
    ...
    function test_TransferFrom() external givenStreamTransferable {
        ...
        vm.expectEmit({ emitter: address(lockup) });
        emit Transfer({ from: users.recipient1, to: users.alice, tokenId: streamId });

        // Transfer the NFT.
        lockup.transferFrom({ from: users.recipient1, to: users.alice, tokenId: streamId });
