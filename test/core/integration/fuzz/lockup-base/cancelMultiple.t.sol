...
abstract contract CancelMultiple_Integration_Fuzz_Test is Lockup_Integration_Shared_Test {
        ...
        givenAllStreamsCancelable
    {
        ...
        if (lockupModel == Lockup.Model.LOCKUP_LINEAR) {
            streamId = createDefaultStreamWithEndTimeLL(endTime);
        } else if (lockupModel == Lockup.Model.LOCKUP_DYNAMIC) {
            streamId = createDefaultStreamWithEndTimeLD(endTime);
        } else if (lockupModel == Lockup.Model.LOCKUP_TRANCHED) {
            streamId = createDefaultStreamWithEndTimeLT(endTime);
        }
        ...
        uint256[] memory streamIds = Solarray.uint256s(cancelMultipleStreamIds[0], streamId);
