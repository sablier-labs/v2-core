...
abstract contract Renounce_Integration_Basic_Test is Integration_Test, Lockup_Integration_Shared_Test {
  ...
  function test_RevertWhen_CallerNotSender() external whenNotDelegateCalled whenStreamWarm {
    ...
    // Run the test.
    vm.expectRevert(abi.encodeWithSelector(Errors.SablierV2Lockup_Unauthorized.selector, defaultStreamId, users.eve));
    lockup.renounce(defaultStreamId);
  }
  ...
  function test_RevertWhen_StreamNotCancelable() external whenNotDelegateCalled whenStreamWarm whenCallerSender {
    ...
    // Run the test.
    vm.expectRevert(abi.encodeWithSelector(Errors.SablierV2Lockup_StreamNotCancelable.selector, notCancelableStreamId));
    ...
    whenRecipientImplementsHook
  {
    ...
    // Expect a call to the hook.
    vm.expectCall(address(revertingRecipient), abi.encodeCall(ISablierV2LockupRecipient.onStreamRenounced, (streamId)));
    ...
    whenRecipientDoesNotRevert
  {
    ...
    // Expect a call to the hook.
    vm.expectCall(address(reentrantRecipient), abi.encodeCall(ISablierV2LockupRecipient.onStreamRenounced, (streamId)));
