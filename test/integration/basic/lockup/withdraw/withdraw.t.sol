...
abstract contract Withdraw_Integration_Basic_Test is Integration_Test, Withdraw_Integration_Shared_Test {
  ...
  function test_RevertWhen_DelegateCalled() external {
    ...
    bytes memory callData = abi.encodeCall(
      ...
      (defaultStreamId, users.recipient, withdrawAmount)
    );
    ...
    whenCallerUnauthorized
  {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        Errors.SablierV2Lockup_InvalidSenderWithdrawal.selector,
        defaultStreamId,
        users.sender,
        users.sender
      )
    ...
    whenWithdrawAmountNotZero
  {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(Errors.SablierV2Lockup_Overdraw.selector, defaultStreamId, MAX_UINT128, withdrawableAmount)
    );
