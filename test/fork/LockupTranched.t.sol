...
abstract contract LockupTranched_Fork_Test is Fork_Test {
    ...
    function testFork_LockupTranched_RebasingAsset() external {
        ...
        lockupTranched.configureRebasingAsset({ asset: IERC20Rebasing(rebasingAsset), yieldMode: YieldMode.CLAIMABLE });

        // Query the yield mode for `ASSET`.
        YieldMode actualYieldMode = lockupTranched.getRebasingAssetConfiguration(IERC20Rebasing(rebasingAsset));
        ...
        if (ERC20RebasingMock(rebasingAsset).symbol().eq("USDB")) {
            // Mint some tokens to the lockupTranched.
            resetPrank({ msgSender: ERC20RebasingMock(rebasingAsset).bridge() });
            ERC20RebasingMock(rebasingAsset).mint(address(lockupTranched), 1000e18);
            ...
            uint256 claimableYieldAmount = lockupTranched.getClaimableRebasingAssetYield(IERC20Rebasing(rebasingAsset));

            // Claim and transfer the yield amount to admin.
            resetPrank({ msgSender: users.admin });
            lockupTranched.claimRebasingAssetYield(IERC20Rebasing(rebasingAsset), claimableYieldAmount, users.admin);
            ...
            claimableYieldAmount = lockupTranched.getClaimableRebasingAssetYield(IERC20Rebasing(rebasingAsset));
