...
contract SablierV2Linear__WithdrawAll__UnitTest is SablierV2LinearUnitTest {
    ...
    function testCannotWithdrawAll__StreamNonExistent__FirstStream() external {
        uint256 nonStreamId = 1729;
    ...
    function testCannotWithdrawAll__StreamNonExistent__LastStream() external {
        uint256 nonStreamId = 1729;
    ...
    function testCannotWithdrawAll__Unauthorized__AllStreams() external {
    ...
    function testCannotWithdrawAll__Unauthorized__FirtStream() external {
        // Make Eve the `msg.sender` in this test case.
        changePrank(users.eve);
    ...
    function testCannotWithdrawAll__Unauthorized__LastStream() external {
        // Make Eve the `msg.sender` in this test case.
        changePrank(users.eve);
        // Approve the SablierV2Linear contract to spend $USD from the `eve` account.
        usd.approve(address(sablierV2Linear), type(uint256).max);
    ...
    /// @dev When the caller of all of the streams is the sender, it should make multiple withdrawals.
    ...
    function testCannotWithdrawAll__WithdrawAmountZero__AllStreams() external {
    ...
    function testCannotWithdrawAll__WithdrawAmountZero__FirstStream() external {
        vm.expectRevert(abi.encodeWithSelector(ISablierV2.SablierV2__WithdrawAmountZero.selector, streamId));
        uint256[] memory streamIds = createDynamicArray(streamId, streamId_2);
    ...
    function testCannotWithdrawAll__WithdrawAmountZero__LastStream() external {
        vm.warp(stream.startTime + TIME_OFFSET);
    ...
    function testCannotWithdrawAll__WithdrawAmountGreaterThanWithdrawableAmount__AllStreams() external {
    ...
    /// it should revert.
    function testCannotWithdrawAll__WithdrawAmountGreaterThanWithdrawableAmount__FirstStream() external {
        uint256 withdrawAmountMaxUint256 = type(uint256).max;
        uint256 withdrawableAmount = WITHDRAW_AMOUNT;
        vm.warp(stream.startTime + TIME_OFFSET);
        vm.expectRevert(
    ...
    /// it should revert.
    function testCannotWithdrawAll__WithdrawAmountGreaterThanWithdrawableAmount__LastStream() external {
        uint256 withdrawAmountMaxUint256 = type(uint256).max;
        uint256 withdrawableAmount = WITHDRAW_AMOUNT;
        vm.warp(stream.startTime + TIME_OFFSET);
        vm.expectRevert(
    ...
    function testWithdrawAll__AllStreamsEnded__Events() external {
        ...
        vm.expectEmit(true, true, false, true);
        emit Withdraw(streamId_2, stream.recipient, withdrawAmount);
        uint256[] memory streamIds = createDynamicArray(streamId, streamId_2);
        uint256[] memory amounts = createDynamicArray(withdrawAmount, withdrawAmount);
        sablierV2Linear.withdrawAll(streamIds, amounts);
    }
    ...
    function testWithdrawAll__StreamsEnded__StreamsOngoing() external {
    ...
    function testWithdrawAll__StreamsEnded__StreamsOngoing__DeleteStreams() external {
        // Approve the SablierV2Linear contract to spend $USD from the `recipient` account.
        usd.approve(address(sablierV2Linear), type(uint256).max);
        // Create the ended stream.
        uint256 stopTime = stream.startTime + TIME_OFFSET;
    ...
    function testWithdrawAll__StreamsEnded__StreamsOngoing__Events() external {
        // Approve the SablierV2Linear contract to spend $USD from the `recipient` account.
        usd.approve(address(sablierV2Linear), type(uint256).max);
        // Create the ended stream.
        uint256 stopTime = stream.startTime + TIME_OFFSET;
        ...
        uint256 withdrawAmount_ongoing = WITHDRAW_AMOUNT;
        vm.expectEmit(true, true, false, true);
        emit Withdraw(endedStreamId, stream.recipient, withdrawAmount_ended);
        vm.expectEmit(true, true, false, true);
        emit Withdraw(ongoingStreamId, stream.recipient, withdrawAmount_ongoing);
        uint256[] memory streamIds = createDynamicArray(endedStreamId, ongoingStreamId);
        uint256[] memory amounts = createDynamicArray(withdrawAmount_ended, withdrawAmount_ongoing);
        sablierV2Linear.withdrawAll(streamIds, amounts);
    }
